...
- **The inference_requests ledger insert runs on `context.WithoutCancel(request ctx)` + 5s timeout** — a client disconnect must not erase the audit row for exactly the request most worth auditing; request-scoped values (request id) are preserved, only cancellation is detached.
